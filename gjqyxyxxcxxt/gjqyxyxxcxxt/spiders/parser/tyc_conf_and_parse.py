...
def get_url(key, param):
    url_dic = {
        ...
        'gqcz_url': 'http://www.tianyancha.com/expanse/companyEquity.json?name=%s&ps=100&pn=1',
        'dcdy_url': 'http://www.tianyancha.com/expanse/mortgageInfo.json?name=%s&ps=100&pn=1',
        'qygx_url': 'http://dis.tianyancha.com/dis/getInfoById/%s.json?'
...
def czxx_parse(json_obj):
    ...
    for obj in json_obj['data']['result']:
        ...
        my_dic['LISUBCONAM'] = obj.get('amount', None)
        if my_dic['LISUBCONAM']:
            my_dic['LISUBCONAM'] = u'%s万元' % my_dic['LISUBCONAM']
...
def gqcz_parse(json_obj):
    if 'items' not in json_obj['data']:
        return 'EQUINFO', []
    EQUINFO = []
    for item in json_obj['data']['items']:
        item_dic = {}
        item_dic['EQUITYNO'] = item.get('regNumber', None)
        item_dic['PLEDGOR'] = item.get('pledgor', None)
        item_dic['BLICTYPE'] = item.get('certifNumberR', None)
        item_dic['BLICNO'] = item.get('certifNumber', None)
        item_dic['PLEDAMUNIT'] = '万人民币'
        item_dic['IMPAM'] = item.get('equityAmount', None)
        item_dic['REGCAPCUR'] = item.get('RegCapCur', None)
        item_dic['IMPORG'] = item.get('pledgee', None)
        item_dic['EQUPLEDATE'] = item.get('regDate', None)
        if item_dic['EQUPLEDATE']:
            timestamp = int(str(item_dic['EQUPLEDATE'])[:10])
            item_dic['EQUPLEDATE'] = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(timestamp))
        item_dic['TYPE'] = item.get('state', None)
        EQUINFO.append(item_dic)
    return 'EQUINFO', EQUINFO

def dcdy_parse(json_obj):
    if 'items' not in json_obj['data']:
        return 'MORTINFO', []
    print json_obj['data']
    MORTINFO = []
    ...
    for item in data['items']:
        ...
        item_dic = {}
        item_dic['MORREGCNO'] = baseInfo.get('regNum', None)
        item_dic['REGIDATE'] = baseInfo.get('regDate', None)

        item_dic['REGORG'] = baseInfo.get('regDepartment', None)
        item_dic['PRICLASECAM'] = baseInfo.get('amount', None)
        item_dic['TYPE'] = baseInfo.get('status', None)
        item_dic['PRICLASECKIND'] = baseInfo.get('type', None)
        item_dic['REGCAPCUR'] = baseInfo.get('amount', None)
        item_dic['WARCOV'] = baseInfo.get('scope', None)
        item_dic['PEFPERFORM'] = None
        item_dic['PEFPERTO'] = None
        overviewTerm = baseInfo.get('overviewTerm', None)
        if overviewTerm:
            res = re.findall(u'(\d{4}.+)至(\d{4}.+)', overviewTerm)
            if res:
                item_dic['PEFPERFORM'] = res[0][0]
                item_dic['PEFPERTO'] = res[0][1]
        item_dic['changeInfoList'] = item.get('changeInfoList', [])
        item_dic['pawnInfoList'] = item.get('pawnInfoList', [])
        item_dic['peopleInfo'] = item.get('peopleInfo', [])
        MORTINFO.append(item_dic)
    return 'MORTINFO', MORTINFO

def qygx_parse(json_obj):
    print '____qygx: ', json_obj
