...
class GsjPhoneSpider(Spider):
    ...
    name = 'ic_data2'
    start_urls = [
        'http://gsxt.zjaic.gov.cn/client/entsearch/toEntSearch'
        ]
    ...
    def parse(self, response):
        yield Request('http://gsxt.zjaic.gov.cn/client/entsearch/list?isOpanomaly=&pubType=1&searchKeyWord=414CE5D139144EF3B6A558F923C3FCF641ACAA163E2F8944', callback=self.parse_list)

    def parse_list(self, response):
        hrefs = response.xpath('//ul[@class="enterprise-info-list"]/li/a/@href').extract()
        for h in hrefs:
